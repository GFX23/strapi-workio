import { footerData } from "../_gql/query/homepage";
...
export const Footer: React.FC<Props> = async ({ locale }) => {
  const r = await apolloClient.query({
    ...
    const {logo, title, downloadHere, links, socialSites, footerText, cornerIcon } = r.data.footer.data.attributes as FooterType;

  console.log(locale)

  return (
    <div className="w-full center-all bg-wblue-500">
      <div className="flex flex-col max-w-[1200px] pt-8 gap-10 items-center xl:items-start w-full px-4 xl:px-0">
        ...
        <div className="flex w-full flex-col xl:flex-row gap-10 items-center">
          ...
          <div className="flex md:gap-5 gap-7 md:flex-row flex-col items-center md:items-start">
            {links?.map((link, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center md:items-start px-5 md:gap-6 gap-4 text-nowrap"
              >
